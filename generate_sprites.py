...
BEAT_PROMPTS = {
    ...
    "brenno_tek_screws": (
        "pixel art DETAILED CLOSE-UP view down into the E36 engine bay at night focused on the "
        "cracked PLASTIC INTAKE MANIFOLD or AIRBOX, "
        "REALISTIC pixel art style matching Moey/Khoa NPC fidelity, "
        "the focal element is the CRACKED BLACK PLASTIC ENGINE-BAY COMPONENT with multiple HEX-HEAD "
        "TEK SCREWS (zinc-plated silver self-tapping screws with hex drive heads + rubber sealing "
        "washers) DRIVEN THROUGH the plastic into the underlying metal in obviously amateur "
        "positions — about FIVE OR SIX tek screws clustered around a crack to hold it together, "
        "the screws sitting proud of the plastic surface with rough sharp tip ends visible inside, "
        "smaller crack-spider lines radiating out from where the screws bite into the plastic, "
        "ONE TEK SCREW MID-INSTALLATION — a small electric IMPACT DRIVER bit visible at the top of "
        "frame caught in the act of driving a NEW tek screw into the plastic, faint blue-white "
        "spark/dust kicking up from the threading action (the animation focal element — the impact "
        "driver spins and the screw turns), "
        "in the foreground a HAND visible — Brenno's pale ginger-freckled hand holding the back of "
        "the impact driver, mustard-yellow jumper cuff visible at the wrist, "
        "a stylised CARTOON BADGE in the upper-right corner reading 'LIFETIME WARRANTY' with a small "
        "stamped seal icon (NO readable text needed — just shape recognition: a circular gold/silver "
        "stamp graphic), "
        "scattered LOOSE TEK SCREWS rolling around the engine bay near the work area, a small "
        "PLASTIC BAG of bunnings-brand tek screws sitting on the strut tower, "
        "the engine bay around shows faded BMW E36 blue paint with grime, polished aluminium valve "
        "cover in the periphery, "
        "harsh workshop droplight casting bright contrast lighting on the screw work, "
        "BACKGROUND: blurred dark Maccas carpark, deep navy night sky at top edge, "
        "16-bit pixel art with HIGH DETAIL on the cluster of tek screws + cracked plastic + impact "
        "driver mid-drive, REALISTIC pixel art NOT chibi, the 'tek screws hold everything together' "
        "ghetto-fix pride moment, no full human characters — only the working hand + tools"
    ),
