...
CHARS = {
    ...
    "preet": (
        ...
        "TALL SLIM LEAN Indian Sikh Punjabi Australian man in his late twenties "
        ...
        "friendly wide grin showing teeth, warm brown skin, prominent angular jawline under the beard, "
        ...
        "kara steel bracelet visible on his right wrist, "
        "wearing a CRISP LIGHT-BLUE BUTTON-UP COLLARED SHIRT (NOT a kurta, NOT a t-shirt, NOT a polo — a proper light-blue dress shirt) "
        "with the sleeves ROLLED UP cleanly to the elbows exposing his forearms, the shirt unbuttoned slightly at the collar, "
        "an OPEN BLACK WAISTCOAT (vest) worn over the blue shirt with no buttons fastened — the waistcoat hangs loose at his sides, "
        "DARK INDIGO BLUE STRAIGHT-LEG JEANS (NOT loose trousers), BROWN LEATHER STRAP SANDALS on his feet, "
        "standing upright at full body height in a relaxed adult pose with hands at sides, "
        "richly shaded with multiple tone bands, defined three-dimensional anatomy, heavy multi-tone pixel shading with at least 4-5 distinct tone bands per surface, "
        "Harris Park west Sydney lovable Punjabi Australian — UberEats driver by night, family man, mainy king"
    ),
    "macca": (
        "REALISTIC GRITTY pixel art full-body character sprite of a real human person, "
...
CARS = {
    ...
    "silvia": "Nissan Skyline R32 GT-R BNR32 two-door coupe seen from directly above straight down, "
           "viewed from a strict bird's eye top-down perspective, CRYSTAL WHITE PEARL paint with subtle "
           "GT-R badge accent at the rear, low slung muscular body with wide flared rear quarter panels, "
           "aftermarket carbon GT-wing spoiler visible from above mounted on the boot, deep-dish gunmetal "
           "Work Meister wheels at all four corners, tinted windows, factory ATTESA AWD JDM tuner build, "
           "Crystal White Pearl polished modified look, RB26DETT twin-turbo straight-six under the bonnet, "
           "rectangular shape oriented vertically with iconic R32 boxy-aggressive silhouette",
...
SIDE_CARS = {
    ...
    "camry_side":  "Toyota Camry XV40 sedan from year 2008, four-door sedan body, "
                   ...
                   "completely flat side-on silhouette, NO top-down, NO 3-quarter view, ONLY pure side profile",
    "silvia_side": "Nissan Skyline R32 GT-R BNR32 two-door coupe from year 1991, factory twin-turbo "
                "RB26DETT inline-six AWD ATTESA performance flagship, "
                "seen from a STRICT DIRECT side profile view, "
                ...
                "the entire car length visible from nose to boot — iconic R32 GT-R boxy-aggressive silhouette "
                "with WIDE FLARED REAR QUARTER PANELS bulging out over the rear wheels (the distinctive "
                "BNR32 GT-R fender flare — wider than a regular R32 GTS-T), short blunt nose, square jaw, "
                "PAIR of round QUAD HEADLIGHTS at the front (four circular headlights total — two per side, "
                "the iconic R32 headlight signature), CRYSTAL WHITE PEARL paint with subtle metallic shimmer "
                "catching the light, "
                "deep-dish GUNMETAL WORK MEISTER aftermarket alloy wheels (multi-spoke), low-profile black "
                "tyres, slammed lowered coilover stance with aggressive negative camber on all four wheels, "
                "aggressive aftermarket front lip splitter beneath the front bumper, aero side skirts, "
                "TALL CARBON-FIBRE GT-WING rear spoiler bolted to the boot lid (factory-spec NISMO style "
                "wing — taller and more aggressive than a small lip), "
                "factory red GT-R badge on the front grille between the headlights, small GT-R badge on "
                "the rear quarter, "
                "tinted side windows, JDM tuner build but kept clean as a daily driver, "
...
BRENNO_CANONICAL = (
    # Modelled closely on the original portrait_bazza_default prompt that
    # worked. Tighter structure, fewer 'CRITICAL' callouts, cap tilted up
    # instead of pulled low per user feedback.
    "REALISTIC photo-quality detailed portrait bust of a young white Australian eshay MAN in his "
    "early twenties (MALE adult, MASCULINE features, strong narrow jaw — NOT female, NOT androgynous, "
    "NOT a feminine soft face), "
    "weathered pale milky complexion with light acne scarring scattered across the cheeks, "
    ...
    "UNSHADOWED, eshay 'tilted-up showing the cap' wearing style), "
    "the bright YELLOW NIKE SWOOSH logo prominently visible on the front-side panel of the cap, "
    "wearing a baggy oversized BRIGHT MUSTARD YELLOW PULLOVER SWEATSHIRT in school-bus-yellow fabric "
    "(absolutely NOT a collared shirt, NOT a button-up, NOT a polo, NOT a blazer, NOT a hoodie, NOT "
    "a t-shirt — a plain crewneck yellow pullover sweatshirt, the kind of jumper a tradie kid wears), "
    "the bright yellow fabric clearly visible filling the bottom half of the portrait at the shoulders "
    "and chest, the crewneck collar visible at the neck (no buttons, no zipper), "
    "a thin tarnished silver chain hanging at the neck above the jumper, "
    "faded blue prison-style knuckle tattoos peeking up from the jumper collar onto the lower neck, "
    "a black Nike Crossbody bum bag strap clearly visible running diagonally across one shoulder "
    "over the yellow jumper, "
)
...
SUBURB_SCENE_PROMPTS = {
    ...
    "cabramatta": (
        ...
        "BESIDE HIM his CRYSTAL WHITE PEARL Nissan R32 GT-R Skyline BNR32 two-door coupe parked low with the boot open showing "
...
# Locked character descriptions for use in beat-GIF prompts. Drop these
# constants into any prompt that features the character — guarantees the
# face/outfit/style stays consistent with the master portrait pipeline.
# When a character description changes, update the constant in one place
# and clear caches for every affected GIF.
BRENNO_LAD_DESC = (
    "Brenno is a wiry pale young white Australian eshay LAD in his early twenties "
    "(MASCULINE adult man face — NOT a cute teenage boy, NOT a chibi figure, NOT anime, "
    "NOT a cartoon — proper Moey/Khoa NPC realism tier), "
    ...
    "(absolutely NOT a button-up, NOT a polo, NOT a hoodie, NOT a t-shirt, NOT a blazer — a plain "
    "crewneck yellow jumper) as his main top garment, "
    "navy tracksuit pants tucked into long white tube socks, white rubber slides on his feet, "
    "a black NIKE CROSSBODY BUMBAG STRAP visible running diagonally across one shoulder over the "
    "yellow jumper, "
    "faded blue prison-style stick-and-poke KNUCKLE TATTOOS visible at the neckline, "
...
KHOA_DESC = (
    ...
    "proper Moey/Khoa NPC realism tier with realistic adult human proportions), "
    "warm tan East-Asian complexion with detailed pore-level skin texture and gritty multi-tone "
    "shading, lean angular jawline, sharp dark brown eyes, well-groomed thick BLACK HAIR styled "
    "neatly with a slight side-part (NOT spiky anime hair, NOT a fringe — proper Cabra plug clean "
    "haircut), "
    "wearing a CRISP LIGHT-BLUE BUTTON-UP COLLARED SHIRT with the sleeves ROLLED UP cleanly to the "
    "elbows exposing his forearms (NOT a t-shirt, NOT a polo, NOT a hoodie — a proper light-blue "
    "dress shirt, the same colour family as Rahul's but worn more casually open at the collar), "
    "a thin DELICATE GOLD CHAIN visible at the open collar, "
    "a small GOLD CAP CROWN on one upper front tooth catching the light when he smiles, "
    "dark indigo well-fitted JEANS, clean white minimalist sneakers, "
    "a slim TAG-HEUER style WATCH on his left wrist with a stainless-steel bracelet, "
    "polite businesslike COOL-OPERATOR body language — calm, confident, NOT loud, NOT eshay-energy, "
    "more JDM-tuner-businessman vibe, "
    "REALISTIC GRITTY hand-painted pixel-art adult NPC fidelity matching the Moey/Khoa portrait "
    "realism bar — heavy multi-tone pixel shading with at least 4-5 distinct tone bands per surface, "
    "absolutely NOT the soft anime/illustrated look, gritty real-world Cabramatta plug aesthetic"
)

RAHUL_DESC = (
    "Rahul is a Sikh Punjabi Australian man in his late twenties "
    "(MASCULINE adult man face — NOT anime, NOT a cartoon, NOT chibi, NOT cell-shaded — "
    "proper Moey/Khoa NPC realism tier with realistic adult human proportions), "
    "warm brown South-Asian complexion with detailed pore-level skin texture and gritty multi-tone "
    "shading, strong defined jawline, friendly intelligent dark brown eyes, "
    "wearing a BRIGHT ROYAL BLUE DASTAR (Sikh turban) cleanly wrapped with crisp visible fabric "
    "creases — the dastar is the iconic colour anchor, "
    "a THICK FULL JET-BLACK BEARD (proper Sikh kesh — uncut, dense, well-groomed, NOT a thin stubble "
    "and NOT a cartoon goatee), the moustache joining the beard cleanly, "
    "wearing a CRISP LIGHT-BLUE BUTTON-UP COLLARED SHIRT with the sleeves ROLLED UP to the elbows "
    "exposing his forearms (NOT a t-shirt, NOT a polo, NOT a hoodie — a proper light-blue dress shirt), "
    "an OPEN BLACK WAISTCOAT (vest) worn over the blue shirt with no buttons fastened, the waistcoat "
    "hanging loose at his sides, "
    "DARK INDIGO BLUE STRAIGHT-LEG JEANS, BROWN LEATHER STRAP SANDALS on his feet (NOT sneakers, "
    "NOT runners), "
    "a thin steel KARA (Sikh bangle) visible on his right wrist, "
    "REALISTIC GRITTY hand-painted pixel-art adult NPC fidelity matching the Moey/Khoa portrait "
    "realism bar — heavy multi-tone pixel shading with at least 4-5 distinct tone bands per surface, "
    "absolutely NOT the soft anime/illustrated look, gritty real-world Western Sydney street style"
)
...
BEAT_PROMPTS = {
    ...
    # wedged in his mouth."
    "curry_emerge": (
        # MID-STRIDE STAGGERING OUT per Michael's reference — side-on
        # cinematic angle, Brenno walking out of the lit grocer doorway
        # carrying a heavy paper bag, samosa wedged in mouth, the doorway
        # + shelves of jars form the warm scene anchor behind him.
        # Realistic gritty atmospheric pixel art.
        "pixel art WIDE FULL-BODY SIDE-ON CINEMATIC SHOT of an Indian grocer shopfront on "
        "Wigram Street Harris Park at night, "
        "centred in the frame is " + BRENNO_SCENE_DESC + ", "
        "FULL BODY visible head to sneakers occupying about 75 percent of frame height, "
        "Brenno is caught MID-STRIDE walking OUT through the OPEN DOORWAY of the grocer — "
        "one foot mid-step forward planted on the footpath outside, the other foot still on "
        "the doorway threshold lifting off, his body leaning slightly forward with the weight "
        "of the grocery bag, "
        "ONE HAND grips the top of a HEAVY BROWN PAPER GROCERY BAG hanging at his hip — the "
        "bag bulges full of contents, you can see the tops of TWO BOTTLES of curry powder + "
        "GARAM MASALA jars + a baguette of rolled-up onion bunch poking out the top of the "
        "bag, the paper bag has a small red 'HOT' sticker visible on its side, "
        "the OTHER HAND swings naturally at his side mid-stride, "
        "a GOLDEN-BROWN TRIANGULAR ALOO SAMOSA is WEDGED IN HIS MOUTH (the focal detail — the "
        "samosa juts out from between his lips with a couple of crumb specks falling), one "
        "corner half-bitten off, his cheeks slightly puffed around it, "
        "BACKGROUND directly behind him through the OPEN DOORWAY — the warm yellow lit "
        "INTERIOR of the grocer shop: floor-to-ceiling SHELVING units lining the back wall "
        "stocked with rows of COLOURFUL SPICE JARS in red, green, yellow, brown lids (the "
        "shelves of jars fill the doorway opening as a warm colourful backdrop), "
        "the doorway is framed by a dark wooden door propped open to the left with a small "
        "yellow shop hours sign tacked to it, the right side of the doorway shows the edge "
        "of a glass shop window with more spice display visible inside, "
        "above the doorway a partial 'NN' shop sign visible (cropped — no readable full text), "
        "FOREGROUND footpath — dark pavement / tile pattern with warm yellow shop light "
        "spilling out onto it from the doorway, illuminating Brenno from the side, "
        "DEEP NAVY night sky barely visible at the top edge of frame, "
        "the warm interior glow contrasts beautifully against the cool night air outside, "
        "AMBIENT LIGHTING — the yellow shop light catches Brenno's mustard jumper and "
        "makes it glow, the navy cap shadowed slightly, "
        ...
        "the cinematic 'mid-stride out of the grocer with mum's shopping' moment, no speech "
        "bubbles, no cartoon overlays — just the atmospheric scene"
    ),
    ...
    "khoa_handoff": (
        ...
        "looking OUT at the parked WHITE PEARL Nissan R32 GT-R Skyline parked beside us, "
        ...
        "Khoa visible just past his open R32 window — Vietnamese Australian man in his late twenties "
        "with crisp light blue button-up shirt, friendly confident polite expression, "
        "the white pearl Skyline body visible in the background with its deep-dish gunmetal wheels, "
    ...
    "brett_handover": (
        ...
        "his HEAD IS BOWED / TILTED DOWN, looking DOWN at the bong in his hands (the downward "
        "gaze deepens the hat-brim shadow over his face), a quiet sentimental reluctant expression "
        "on his visible lower jaw, "
        ...
        "Brett is HANDING OVER a tall GLASS DOUBLE PERCOLATOR BONG held in BOTH HANDS, extending it "
        "FORWARD and slightly DOWN toward the camera/recipient — a reluctant, careful, two-handed "
        "handover (like passing on something precious, NOT a triumphant trophy-raise), "
    ...
    "brenno_plead": (
        # CHEST-UP CLOSE-UP per Michael's reference — bust framing with
        # both hands clasped at the chin in praying/begging pose, mouth
        # open mid-plea, wide pleading eyes, heavy freckles, thought
        # bubble beside head showing the Westfield-smoking context.
        # Dark night vibe with railing/fence in background.
        "pixel art CHEST-UP BUST CLOSE-UP REALISTIC shot of " + BRENNO_SCENE_DESC + ", "
        "BUST FRAMING — Brenno's CHEST + ARMS + HEAD fill the centre of the frame, the figure "
        "occupies most of the composition with only a slim margin of background visible at the "
        "edges, "
        "BOTH HANDS CLASPED TOGETHER in front of his MOUTH / under his chin in a PRAYING / "
        "BEGGING POSE (palms together pointing up at his chin like a 'pretty please' clasp), "
        "fingers pressed flat against each other, "
        "his MOUTH IS OPEN MID-PLEA showing teeth, lips slightly parted in a desperate 'PLEEASE "
        "bra' shape, "
        "his EYES are WIDE PLEADING with puppy-dog desperation, eyebrows raised HIGH into his "
        "fringe, "
        "the masculine adult face shows HEAVY DENSELY-PACKED ORANGE FRECKLES across the nose + "
        "cheeks + forehead (the freckles are very prominent in this close-up), "
        "head tilted SLIGHTLY DOWN looking up at the player camera — submissive pleading angle, "
        "bright ORANGE-RED hair visible under and around the cap, "
        "STYLISED THOUGHT BUBBLE floating to the upper-RIGHT of his head — a small cloud-shaped "
        "bubble showing a tiny silhouette of WESTFIELD MALL with a GLASS BONG icon + small red "
        "HEART icons radiating outward (visual storytelling: 'the boys at Westfield are smoking "
        "without me'), the thought bubble has 2-3 small connecting circles trailing down to "
        "his head, "
        "BACKGROUND — dark navy night scene with a HORIZONTAL METAL RAILING / FENCE silhouette "
        "behind Brenno at chest height (suggesting he's leaning against a Wigram Street "
        "footpath railing), a STREETLAMP POLE on the left side with a small yellow shop-sign "
        "plate reading 'PLEASY' on it (a flavour Easter-egg sign — only readable text), the "
        "rest of the background is in deep navy shadow with faint hints of distant shopfront "
        "glow, "
        "AMBIENT LIGHTING — soft warm streetlamp glow illuminating Brenno's face + jumper from "
        "above, the yellow jumper catches the warmth, "
        "16-bit pixel art ATMOSPHERIC REALISTIC GRITTY style with HEAVY multi-tone shading + "
        "high contrast, REALISTIC adult human face (NOT cute kid, NOT chibi, NOT anime, NOT "
        "cartoon), the desperate 'PLEEEEASE bra just one more stop' moment told through the "
        "clasped hands + pleading face + Westfield thought bubble"
    ),
    ...
    "khoa_old_pacific": (
        "pixel art DYNAMIC ACTION side-on view of Khoa's WHITE PEARL Nissan R32 GT-R Skyline mid-CORNER on the "
        "OLD PACIFIC HIGHWAY — a sweeping windy forest road north of Sydney at GOLDEN HOUR Sunday morning, "
        "the R32 in the foreground centre tilted slightly LEANING into a corner with a hint of TAIL-OUT "
        ...
        "16-bit pixel art with HIGH DETAIL on the drifting R32 + golden sunlight, "
    ...
    "khoa_deal_hand": (
        "pixel art EXTREME CLOSE-UP from inside Khoa's white R32 GT-R Skyline at night in Cabra Maccas carpark, "
        "camera angle looking down into the centre console / passenger seat area, "
        "the FOCAL ELEMENT is Khoa's HAND HOLDING UP a small CLEAR ZIP-LOCK BAGGIE loosely filled "
        "with LOOSE CHOPPED-UP GREEN HERB — think finely CUT GRASS / ground dried oregano / shredded "
        "green leafy material, a loose pile of small irregular GREEN FLAKES and shreds (medicinal "
        "green herb, fine and legal), "
        "ABSOLUTELY NOT round cabbages, NOT brussels-sprouts, NOT spheres, NOT spirals, NOT swirls — "
        "it is LOOSE GRANULAR CHOPPED GREEN MATERIAL like a bag of cut grass clippings, settling "
        "loosely in the bottom of the clear baggie, "
        "varied SHADES OF GREEN from light lime through deep forest green, dry crumbly leafy "
        "texture, a few tiny lighter flecks through it, "
        "the bag about half-full of the loose green flakes, the clear plastic catching the light, "
        "the baggie tilted at an angle to camera so the loose chopped-herb texture reads clearly, "
        "Khoa's hand visible — warm-tan Vietnamese Australian man's hand with gritty pore-level skin "
        "detail and heavy multi-tone pixel shading (NOT a flat anime hand, NOT cartoon — proper "
        "realistic adult masculine hand), crisp light-blue button-up shirt sleeve rolled cleanly to "
        "the wrist with visible fabric weave texture, a slim TAG-HEUER style watch with stainless-"
        "steel bracelet visible at the cuff, thumb and forefinger pinching the top of the baggie to "
        "dangle it forward, "
        ...
        "the dashboard wood-grain visible at the bottom of the frame, R32 interior trim, gear shifter "
        ...
        "16-bit pixel art with HIGH DETAIL on the LOOSE CHOPPED GREEN HERB (cut-grass texture, NOT "
        "round buds) + scale display + cash, no human faces, ONLY the hand + product + counter-top "
        "items in close-up"
    ),
    # khoa_geton_q: "YEEEEEEEEW! Q the boys up adlay! ESHAYS. That'll
    # do us all weekend." — Brenno fist-pumping mid-YEEEEEW celebration.
    "brenno_yew_hype": (
        # EXPLICIT CELEBRATION ACTION POSE — Brenno mid-air shouting YES,
        # arms thrown up, paper bag of Q raised triumphantly. The current
        # canonical-pose render was too static for the dialogue energy.
        # This prompt FRONTLOADS the action and explicitly forbids the
        # standing pose drift.
        "pixel art REALISTIC FULL-BODY WIDE ACTION CELEBRATION shot — "
        "central figure is Brenno mid-CELEBRATION exploding with PURE STOKED ESHAY JOY in the "
        "Cabramatta Maccas carpark at night, "
        "POSE — this is NOT a standing portrait, NOT a neutral stance, NOT arms-at-sides — "
        "Brenno is caught IN MID-ACTION with body fully animated: he is mid-leap with both "
        "FEET LIFTED slightly off the asphalt, body extended UPRIGHT with both arms THRUST UP "
        "INTO THE AIR overhead in an EXPLOSIVE DOUBLE-FIST-PUMP, mouth WIDE OPEN in a huge "
        "open-mouthed YEEEEW shout, head TILTED BACK with eyes scrunched in elation, "
        "ONE HAND held HIGH overhead clutching a small BROWN PAPER GROCERY BAG (the Q of green "
        "stashed inside, top crumpled closed) raised triumphantly like a trophy, "
        "the OTHER HAND in a clenched FIST punching straight UP into the air, "
        "CHARACTER DETAILS — " + BRENNO_SCENE_DESC + ", "
        "FULL BODY visible head to slides occupying about 70 percent of frame height with both "
        "arms extended fully into the upper frame (his arms reach above the top edge of the "
        "frame at full extension), masculine adult face with bright orange-red hair under "
        "navy TN cap, "
        "ACTION ENERGY EFFECTS — small stylised radial pixel-art motion lines + a few subtle "
        "white star-sparkle bursts radiating outward from his raised arms emphasising the "
        "EXPLOSIVE celebration moment, faint dust scuff under his feet from the lift-off, "
        "BACKGROUND — Khoa's WHITE PEARL Nissan R32 GT-R Skyline parked side-on visible behind "
        "him (just the front quarter visible at the edge of the frame), Cabra Maccas drive-thru "
        "in the middle distance with the iconic golden ARCHES M LOGO silhouette glowing yellow "
        "on a tall pole, painted white parking bay lines, scattered abandoned shopping trolleys "
        "further off, warm amber carpark lamps casting pools on the asphalt, deep navy night "
        "sky above, "
        "16-bit pixel art GRITTY REALISTIC ATMOSPHERIC style with HEAVY multi-tone shading, "
        "REALISTIC pixel art (NOT chibi, NOT anime, NOT cartoon) — Moey/Khoa NPC realism, "
        "the explosive 'YEEEEW Q THE BOYS UP' celebration moment told through the mid-air "
        "double-fist-pump + raised paper bag — Brenno MUST be in an active celebration pose, "
        "NOT standing still"
    ),
    ...
    "khoa_spotlight_brief": (
        # KHOA BRIEFING per Michael's reference — Khoa standing in front
        # of his R32 with raised index finger ('rule one'), small clock
        # thought bubble beside head showing the timing context, cop
        # silhouette in distant background, single tall streetlamp,
        # atmospheric night briefing scene.
        "pixel art ATMOSPHERIC SQUARE COMPOSITION of " + KHOA_DESC + ", "
        "standing CENTRE-FRAME in the Cabra Maccas carpark at night, leaning casually back "
        "against the driver-side door of his WHITE PEARL Nissan R32 GT-R Skyline parked "
        "behind him (the R32 occupies the middle-ground filling the lower-left third of frame "
        "with its rounded headlights, big wheels, and silvery body clearly visible), "
        "Khoa is in a CASUAL BRIEFING POSE — his RIGHT INDEX FINGER raised high in a 'rule "
        "one' / 'listen up' gesture pointed up toward the sky, body slightly tilted, slight "
        "smirk on his masculine adult face, gold chain at the open collar of his crisp light-"
        "blue button-up shirt catching the streetlamp glow, "
        "FULL TORSO + head + raised arm visible (about 75 percent of frame height), face "
        "small in frame so PixelLab can't drift, "
        "BESIDE HIS RAISED FINGER — a small STYLISED THOUGHT-BUBBLE icon floating in the "
        "upper-right of frame showing a tiny ANALOGUE CLOCK face with hands at about 10:30 + "
        "a faint red countdown segment (the 'we're on a timer' context — the focal animation "
        "element pulses gently), "
        "BACKGROUND — far away in the distant background a silhouetted UNIFORMED FOOT-PATROL "
        "COP (Sgt Snout style human cop with subtle pig snout) standing beside the silhouette "
        "of a parked NSW Highway Patrol Commodore with its spotlight off — they are SMALL and "
        "DISTANT (atmospheric, not the focal subject), "
        "a single TALL STREETLAMP POLE on the LEFT edge of frame with a warm yellow lamp at "
        "the top casting a soft pool of light onto Khoa + the R32, "
        "deep dark navy/black sky overhead, distant gum-tree silhouettes along the far back "
        "fence, a horizontal METAL GUARD-RAIL silhouette running across the middle ground "
        "behind Khoa separating the carpark from the road beyond, "
        "AMBIENT LIGHTING — warm streetlamp glow falls on Khoa from above-left, the R32's "
        "white pearl paint reflects the lamp warmly, the rest of the scene is in deep navy "
        "shadow, atmospheric high-contrast lighting, "
        "16-bit pixel art ATMOSPHERIC REALISTIC GRITTY style with HEAVY multi-tone shading, "
        "REALISTIC pixel art (NOT chibi, NOT anime, NOT cartoon) — Moey/Khoa NPC realism, "
        "the calm businesslike 'here's how we do this' briefing moment told through Khoa's "
        "raised-finger gesture + the clock thought bubble + the atmospheric night vibe"
    ),
    ...
    "khoa_slick_celebration": (
        # OPEN-BOOT STASH SHOT per Michael's reference — 3/4 down view of
        # the R32's open boot showing the stash safely transferred. No
        # human characters in frame — atmospheric still life of the
        # successful handoff. Registration plate visible at the bottom.
        "pixel art ATMOSPHERIC DOWN-ANGLED 3/4 view of the OPEN BOOT of the player's car at "
        "night in the Cabra Maccas carpark, looking down into the boot interior, "
        "the BOOT LID is FULLY PROPPED OPEN above frame with the iconic R32-style boot "
        "fame visible at the upper edges of the shot, the open boot REVEALS the dark "
        "carpeted boot floor in the centre of the composition, "
        "INSIDE THE BOOT FLOOR — the FOCAL ELEMENTS sit in a neat arrangement: "
        "(1) a SMALL CLEAR ZIP-LOCK BAG of bright green CANNABIS NUGS sitting on the carpet — "
        "the bag is partly visible with the green nug-shaped buds clearly readable inside "
        "(elongated cone-shaped buds with orange-red pistil hairs and white trichome dusting, "
        "NOT cabbages, NOT vegetables), "
        "(2) a small SPARE TYRE / rim mounted in the side recess of the boot, "
        "(3) a BROWN CARDBOARD BOX with stylised printed text on the side reading 'EAT-PIZZA' "
        "or similar (a takeaway-style box used as cover-prop), "
        "(4) a couple of GOLDEN-BROWN CROISSANTS / pastries sitting beside the box (cover-"
        "innocent-looking grocery items hiding the stash in plain sight), "
        "(5) a small dark fabric DUFFLE BAG with a zip pull-tab, "
        "the items are arranged casually but deliberately on the carpet — they tell the story "
        "of a successful stealth handoff, "
        "REAR EDGE OF FRAME — the REAR BUMPER + LICENCE PLATE visible at the very bottom edge "
        "of the composition, the plate reading 'A56813' in clean stylised text on a white "
        "Australian plate background (the only readable text), the round red R32 tail-lights "
        "partially visible on either side of the plate at the bottom corners of frame, "
        "BACKGROUND PERIPHERY — the dark Cabra Maccas carpark asphalt visible behind the "
        "open boot, painted yellow parking bay lines, faint distant streetlamp pools visible "
        "in the deep background, deep navy night sky, the McDonalds golden ARCHES silhouette "
        "tiny and out-of-focus far in the distance, "
        "AMBIENT LIGHTING — the boot interior is dimly lit by a small boot-lamp glow + the "
        "ambient streetlamp orange from above, HEAVY contrast between the lit boot interior "
        "and the dark exterior beyond, atmospheric high-contrast lighting, "
        "16-bit pixel art ATMOSPHERIC REALISTIC GRITTY style with HEAVY multi-tone shading + "
        "high contrast, REALISTIC pixel art (NOT chibi, NOT anime, NOT cartoon), the "
        "'we got away with it, here's the stash' still-life moment told through the open-boot "
        "atmospheric composition — NO human characters in frame, just the boot + stash + plate"
    ),
    ...
    "brenno_grocer_dash": (
        ...
        "camera with a half-grin on a masculine adult face, mouth open mid-yell, "
    ...
    "rahul_greet_welcome": (
        ...
        + RAHUL_DESC + ", "
        "standing on the footpath with both arms OUTSTRETCHED in a wide welcoming gesture, palms up, "
        "head slightly tilted back, warm friendly grin showing teeth on a masculine adult face, "
        "FULL BODY visible head to sandals occupying about 70 percent of frame height, "
    ...
    "chamkili_720_spin": (
        "pixel art REALISTIC GRITTY hand-painted side-on view of Rahul's grey Toyota Camry 'Chamkili' "
        "STATIONARY in the middle of Wigram Street Harris Park at night, doing a HUGE BURNOUT — "
        "front wheels SPINNING violently with MASSIVE BILLOWING WHITE TYRE SMOKE pouring out from "
        "BOTH FRONT WHEEL WELLS, "
        "RICH MULTI-TONE PIXEL SHADING with at least 4-5 distinct tone bands on every painted "
        "surface, gritty real-world car detail matching the Moey/Khoa NPC realism bar — absolutely "
        "NOT a soft anime illustration, NOT a flat-colour cartoon, NOT a cell-shaded look, NOT a "
        "smooth Pixar render — proper crunchy 16-bit pixel-art aesthetic with hard pixel edges and "
        "heavy contrast shadows, "
    ...
    "rahul_shock_amazed": (
        "pixel art MEDIUM-WIDE three-quarter view of " + RAHUL_DESC + ", "
        "standing on Wigram Street Harris Park at night, FULL TORSO and head visible (NOT just a "
        "face close-up — show waistcoat, shirt, and shoulders in frame), "
        "caught MID-REACTION of GENUINE STUNNED AMAZEMENT — "
        ...
        "MOUTH OPEN in a SHOCKED SMILE showing teeth on a masculine adult face — the rare combination "
        "of disbelief + joy + 'I cannot believe what I just saw', eyebrows raised HIGH, "
    ...
    "rahul_arrival_door": (
        "pixel art side-on REALISTIC view of Wigram Street Harris Park at night, "
        ...
        + RAHUL_DESC + ", "
        "caught MID-STEP HOPPING OUT of the open driver's door — ONE FOOT already planted on the "
        "asphalt, the OTHER LEG mid-air still swinging out from inside the car, his BODY leaning "
        "forward in the motion of standing up, FULL BODY visible head to sandals (NOT just leaning "
        "stationary against the car — clear hopping-out motion), "
    ...
    # khoa_explain — "RB26DETT, twin Garretts, front mount, NISMO Cross 6-speed,
    # conversion, hydraulic handbrake for the touges. She makes a smooth
    # 380 to the wheels."
    "khoa_s14_engine_bay": (
        "pixel art DETAILED 3/4 view down into the OPEN ENGINE BAY of Khoa's WHITE PEARL Nissan R32 "
        "Skyline at night in the Cabra Maccas carpark, the BONNET PROPPED OPEN above the frame, "
        "the engine bay DOMINATES the composition — a built NISSAN RB26DETT INLINE SIX-CYLINDER TWIN-TURBO "
        "engine in a longitudinal rear-wheel-drive layout: BLACK CAST-IRON BLOCK with a polished "
        "twin polished aluminium VALVE COVERS stamped 'NISSAN RB26DETT' in raised letters, "
        ...
        "GLOSSY VINYL DECAL on the inside of the bonnet reading 'RB26DETT' in stylised JDM-style script "
        "+ '500WHP RB26', "
        "below the engine peeking through gaps: the top of the NISMO Cross 6-speed gearbox bell-housing visible "
        ...
        "+ RB26DETT decal, the 'RB26DETT twin Garretts NISMO 6-speed' GT-R enthusiast moment, no human characters"
    ),
    # khoa_geton_g — "All good bro. Pop the boot — bag's in the brown
    # paper from the dosa place. Anyone asks, ya bought a samosa."
    "khoa_boot_brown_bag": (
        "pixel art DOWN-ANGLED 3/4 view looking INTO the OPEN BOOT of Khoa's WHITE PEARL Nissan R32 "
        "Skyline at night in Cabra Maccas carpark, the BOOT LID FULLY PROPPED OPEN above the frame, "
    ...
    "khoa_humble_spotlight": (
        ...
        "R32 GT-R Skyline in the Cabra Maccas carpark at night, "
        ...
        "smile, thin gold chain at the collar) leaning his shoulder back against the R32's rear "
        ...
        "the R32 GT-R Skyline parked side-on beside Khoa with slammed coilover stance, deep-dish gunmetal "
        "wheels, "
    ...
    # white R32 Silvio, vaping and scrolling his phone."
    "cabra_john_st_arrival": (
        ...
        "R32 GT-R Skyline parked at the curb — Khoa is a Vietnamese Australian man in his late twenties, "
        ...
        "leaning his lower back against the R32's rear quarter panel with one foot up against the "
        ...
        "the R32 GT-R Skyline parked side-on, slammed coilover stance, deep-dish gunmetal aftermarket "
        ...
        "+ pearl white R32, the 'Cabra plug at his office' introduction moment, "
    ...
    "brenno_wires_proud": (
        # MINIMALIST DARK ATMOSPHERIC composition per Michael's reference —
        # single focal detail, mostly empty negative space, deep navy/black
        # palette, high contrast shading. NO characters, NO speech bubbles,
        # NO cartoon overlays. Just the wires as a quiet still-life moment.
        "pixel art MINIMALIST ATMOSPHERIC close-up view down into a small section of Brenno's "
        "E36 engine bay at night, "
        "the entire composition is DEEP NAVY-BLACK with HIGH CONTRAST subtle shading — mostly "
        "EMPTY NEGATIVE SPACE around a single focal detail (the frame is dominated by the dark "
        "engine bay surface, very minimal busy detail), "
        "the FOCAL ELEMENT is a SINGLE LOOSE WIRE drooping out of the engine bay loom — one "
        "frayed copper end exposed where it has come loose from its connector, a small WHITE "
        "PLASTIC scotch-lok crimp connector dangling halfway down, the cable insulation is "
        "deep red, the rest of the engine bay is in deep dark shadow with only the faintest "
        "outline of components visible, "
        "a SINGLE tiny BLUE-WHITE ELECTRICAL ARC SPARK animates intermittently at the exposed "
        "copper end (subtle, just a small flicker — the focal animation element), a thin curl "
        "of pale grey smoke rises slowly from the spark point, "
        "deep DARK NAVY-BLACK colour palette overall — almost monochrome, with the only colour "
        "accents being the red wire insulation, the faint blue-white spark, and a hint of warm "
        "amber on the metal edges catching the workshop droplight, "
        "high-contrast hard pixel-art shading with at least 4-5 distinct tone bands per "
        "surface, REALISTIC GRITTY pixel art style, "
        "the composition is QUIET and ATMOSPHERIC — a still-life moment of one dodgy wire "
        "doing its job, NO human characters, NO speech bubbles, NO cartoon labels, NO "
        "lightning-bolt overlays — just the wire detail in the darkness"
    ),
    ...
    # out of six."
    "brenno_points_coil": (
        # WIDE CINEMATIC ESTABLISHING SHOT per Michael's reference — Brenno
        # bent over the open bonnet of his blue E36 from the side, full
        # scene context: dusk sky, streetlamps, dropped tools, beer can.
        # Realistic gritty atmospheric pixel art (ollece-style). The
        # cracked coil pack is INSIDE the engine bay, not the focal subject
        # — the focal subject is the whole atmospheric scene.
        "pixel art WIDE CINEMATIC SIDE-ON ESTABLISHING SHOT of the Blacktown Maccas drive-thru "
        "carpark at dusk — atmospheric realistic gritty pixel art, "
        "centred in the frame is a faded DEEP BLUE BMW E36 sedan parked side-on (facing left) "
        "with the BONNET FULLY PROPPED OPEN above the engine bay, "
        + BRENNO_SCENE_DESC + ", "
        "Brenno is BENT FORWARD AT THE WAIST leaning HEAD-FIRST into the open engine bay from "
        "the front-right side of the car, his back to the camera as he peers down at the M52 "
        "coil packs, both hands resting on the front of the engine bay, mustard yellow jumper "
        "bright against the dusk light, navy trackies, white socks + slides visible, "
        "his FULL BODY is visible in the side-on composition along with the entire E36 — wide "
        "framing, neither figure nor car dominates, both share the frame equally, "
        "the open bonnet props up above on a metal stay, the M52 inline-six engine bay visible "
        "in shadow under the bonnet with a faint glimpse of the row of black coil packs (the "
        "broken cylinder-4 one barely visible inside — atmospheric not diagrammatic), "
        "SCATTERED ON THE ASPHALT around the E36's front wheels — a CHROME RING-SPANNER WRENCH "
        "lying flat, a tipped-over BUNDABERG RUM bottle on its side, an empty BUNDABERG GINGER "
        "BEER can, a discarded oily rag, a small open toolbox with sockets, "
        "BACKGROUND: faint NSW Maccas drive-thru restaurant facade in the middle ground with "
        "the iconic golden ARCHES M LOGO silhouette on a tall pole glowing yellow against the "
        "sky, low-slung brown brick building with lit dining-room windows, drive-thru lane "
        "markings visible on the asphalt, "
        "DRAMATIC DUSK SKY — deep red-purple-pink gradient on the horizon transitioning up to "
        "deep navy at the top, a few stars beginning to appear, distant gum-tree silhouettes "
        "along the back fence, "
        "warm amber STREETLAMPS lit along the carpark edge casting soft pools of light on the "
        "asphalt around the E36, the streetlamp beside the car illuminates Brenno's yellow "
        "jumper from above, "
        "REGISTRATION PLATE on the E36 reading 'KH 1726' in stylised text, "
        "16-bit pixel art CINEMATIC ATMOSPHERIC composition with HIGH DETAIL on the whole "
        "scene — the wide side-on framing, the dropped tools telling a story of late-night "
        "diagnostics, the dusk colour gradient, the streetlamp warmth, "
        "REALISTIC GRITTY pixel-art style (NOT chibi, NOT anime, NOT cartoon, NOT diagrammatic), "
        "the 'diagnostic moment' as a quiet atmospheric scene, no speech bubbles, no labels, "
        "no overlays — just the moment"
    ),
    ...
    "brenno_card_declined": (
        # SIMPLER ATMOSPHERIC composition per Michael's reference — Brenno
        # standing realistic at Maccas at night, half-body waist-up shot,
        # masculine adult face, no phone-screen overlays. The dejected
        # mood is in the body language + the Maccas backdrop, not in
        # cartoon prop overlays.
        "pixel art REALISTIC ATMOSPHERIC half-body waist-up shot of " + BRENNO_SCENE_DESC + ", "
        "standing in front of the Blacktown Maccas drive-thru restaurant at night, "
        "WAIST-UP framing (the shot crops at his lower torso so we see his chest + arms + head "
        "clearly but the figure doesn't dominate the frame — the Maccas backdrop and night "
        "atmosphere are equally important), "
        "Brenno standing front-on slightly angled to camera, body language is DEJECTED RESIGNED "
        "— shoulders slumped, head tilted slightly DOWN with chin to chest, weight on one leg "
        "in a tired slouch, BOTH HANDS pushed deep into the front pockets of his trackies "
        "(NOT holding a phone, NOT holding cash — just hands in pockets, the 'broke and "
        "embarrassed' resigned posture), faint sheepish embarrassed expression on his "
        "masculine adult face, mouth in a tight line, eyes looking down or to the side, "
        "the MUSTARD YELLOW JUMPER catches the warm interior glow from the Maccas windows + "
        "the cool blue night air, the cap with yellow swoosh visible tilted up, "
        "BACKGROUND: the lit-up BLACKTOWN MACCAS restaurant facade fills the middle-ground "
        "behind him — low-slung brown brick building with the warm yellow interior glow "
        "spilling through tall windows, the iconic LARGE BRIGHT YELLOW DOUBLE-ARCH M LOGO "
        "glowing on a tall metal pole to one side (NO readable text on signage — just the "
        "recognisable golden-arches silhouette), drive-thru lane marking on the asphalt, "
        "deep navy NIGHT sky above with a scattering of stars (NOT dusk — full night to suit "
        "the dejected mood), warm amber streetlamp pools on the asphalt around his feet, "
        "subtle atmospheric details — a crumpled receipt blowing past on the asphalt, dim red "
        "drive-thru menu board glow at the side of frame, "
        "16-bit pixel art ATMOSPHERIC realistic style with HIGH DETAIL on Brenno's dejected "
        "body language + the lit-up Maccas behind him, REALISTIC GRITTY pixel art NOT chibi NOT "
        "anime NOT cartoon, the QUIET MOMENT of 'card declined, mum's waiting' told through "
        "body language and atmosphere — NO phone screen overlay, NO cartoon prop icons, NO "
        "speech bubbles, just the moment"
    ),
    ...
    "brenno_onions_dont_wait": (
        ...
        "KITCHEN scene, "
        "Brenno is standing with BOTH HANDS CLASPED TOGETHER under his chin in PRAYING/BEGGING pose, "
        "his pale-blue eyes wide in desperate pleading, eyebrows raised high, mouth open mid-plea, "
        "body slightly bent forward at the waist, "
    ...
    "evo_gps_penrith_route": (
        # INTERIOR DRIVER POV showing the Evo dashboard + GPS map for the
        # post-Khoa-deal travel plan: 'Penrith next, then Liverpool'.
        # Same look as player_route_commit but with the Cabra->Penrith->
        # Liverpool route + atmospheric night driving vibe.
        "pixel art REALISTIC ATMOSPHERIC interior view from the BACK SEAT of the player's "
        "WHITE Mitsubishi Lancer EVO IX looking FORWARD toward the dashboard at night, "
        "the player visible from BEHIND in the driver's seat on the RIGHT (Australian RHD — NOT "
        "face — back of head and shoulders only), short cropped dark brown hair, BLACK ZIP-UP "
        "HOODIE on the shoulders, both hands on the BLACK aftermarket MOMO-style STEERING WHEEL, "
        "head tilted slightly forward focused on the road ahead, "
        "in the PASSENGER SEAT on the LEFT sits BRENNO, visible from behind/side — a wiry young "
        "eshay lad in a BAGGY MUSTARD YELLOW CREWNECK JUMPER (bright yellow, clearly visible), a "
        "NAVY BLUE NIKE TN CAP tilted UP on the back of his head with the yellow swoosh, BRIGHT "
        "ORANGE hair and a thin orange RAT-TAIL on his neck poking out under the cap, slouched "
        "in the seat looking at the GPS phone, one hand resting on the centre console — the "
        "yellow jumper + orange rat-tail clearly identify him as Brenno, "
        "the EVO DASHBOARD is the FOCAL ELEMENT — the centre of the frame is dominated by the "
        "dashboard phone-mount holding the player's smartphone in PORTRAIT orientation, the "
        "phone screen displays a stylised GPS NAVIGATION APP view at night: "
        "a DARK NAVY MAP background with a bold GLOWING ORANGE DASHED ROUTE LINE drawn across "
        "it — the route starts at a circular 'A' pin in the bottom-right corner labelled "
        "'CABRA', winds north-west to a 'B' pin labelled 'PENRITH' near the top-left of the "
        "map, then continues south to a 'C' pin labelled 'LIVERPOOL' in the lower-left "
        "(three pins total — A=Cabra, B=Penrith, C=Liverpool — only readable labels), "
        "a small COIL PACK icon graphic at the C/Liverpool pin (stylised black plug shape, no "
        "words), a small TYRE/UTE icon at the B/Penrith pin (stylised), "
        "an estimated time '47 MIN' floating above the route in stylised yellow digital text, "
        "below the GPS the EVO's iconic dashboard cluster — twin round analogue gauges "
        "(speedo on the left, tacho on the right) with WARM ORANGE-AMBER NEEDLE GLOW visible "
        "behind the steering wheel, fuel + temp + boost gauges visible, a small dangling "
        "charm hanging off the rear-view mirror at the top edge of frame, "
        "the BLACK MOMO STEERING WHEEL visible at the bottom of the frame with the player's "
        "hands gripping it at 9 and 3, "
        "through the WINDSCREEN faintly visible beyond the dashboard — a NIGHT MOTORWAY view "
        "with tail-lights of distant cars ahead glowing red, lane markings stretching forward, "
        "deep navy night sky above with a few distant city lights on the horizon, no readable "
        "text on any background signage, "
        "AMBIENT LIGHTING — warm AMBER dashboard glow illuminates the player's hands + the "
        "interior trim from below, the phone-screen GPS map casts a soft cyan-orange glow on "
        "the dashboard around it, atmospheric warm-vs-cool colour contrast, "
        "16-bit pixel art ATMOSPHERIC REALISTIC GRITTY style with HEAVY multi-tone shading, "
        "REALISTIC pixel art (NOT chibi, NOT anime, NOT cartoon) — Moey/Khoa NPC realism, "
        "the QUIET POST-DEAL COCKPIT MOMENT of 'Penrith next, then Liverpool' told through "
        "the dashboard tech + GPS route + hands on the wheel, no speech bubbles, no overlays"
    ),
    "player_route_commit": (
    ...
    "brenno_yew_legend": (
        # MID-AIR ACTION CELEBRATION per Michael's reference — full-body
        # mid-jump with arms thrown up in V-cheer, big open-mouth shout,
        # DIRT/DUST SPLASH plume kicking up beneath his feet (the focal
        # animation element), city skyline silhouette + dusk sky behind.
        # Realistic gritty pixel art. No speech bubble overlays — the
        # body language tells the story.
        "pixel art REALISTIC FULL-BODY WIDE ACTION shot of " + BRENNO_SCENE_DESC + ", "
        "caught MID-AIR at the APEX of a triumphant CELEBRATION JUMP in an empty Western "
        "Sydney carpark at dusk, "
        "Brenno fills the centre of the frame with FULL BODY visible head to slides (about "
        "70 percent of frame height), proper ADULT lad proportions (NOT a child-sized figure, "
        "NOT chibi NOT anime), "
        "BOTH FEET FULLY OFF THE GROUND — he is airborne at the peak of the jump, body fully "
        "extended upright, legs slightly bent at the knees with feet relaxed (slides on his "
        "feet), the ground is clearly BELOW HIM not touching, "
        "BOTH ARMS THROWN UP overhead in a victorious WIDE V-SHAPE — fists clenched and "
        "raised high above his head, elbows extended, "
        "head TILTED BACK with mouth OPEN WIDE in a huge full-throated CHEER / shout of pure "
        "ESHAY joy, eyes scrunched in elation, masculine adult face animated with celebration, "
        "BENEATH HIS FEET — a BIG ATMOSPHERIC DIRT/DUST SPLASH plume bursting upward from the "
        "asphalt where he just leapt off, the splash is rendered as a dramatic earthy "
        "orange-brown cloud erupting outward in radial arcs (the focal animation element — "
        "the splash plume erupts then dissipates), small chunks of dirt + asphalt fragments "
        "kicking up in the splash, "
        "BACKGROUND — atmospheric Western Sydney CITY SKYLINE SILHOUETTE filling the middle-"
        "ground: blocky industrial buildings + warehouse silhouettes + a single tall streetlamp "
        "on a pole to one side, distant low-rise rooftops, all rendered as DARK NAVY-PURPLE "
        "silhouettes against the sky, "
        "DUSK SKY — deep purple-navy gradient from the horizon up to the top of the frame "
        "with a hint of warm pink-orange glow at the far horizon, a few scattered stars "
        "beginning to appear, "
        "the asphalt beneath is a dark greyish-purple in shadow, faint painted yellow parking "
        "bay lines visible to the sides, "
        "subtle ATMOSPHERIC LIGHTING — Brenno's yellow jumper catches the last warm light "
        "from the horizon, the cap with yellow swoosh catches a glint, "
        "16-bit pixel art ATMOSPHERIC REALISTIC GRITTY style with HEAVY multi-tone shading, "
        "REALISTIC pixel art (NOT chibi, NOT anime, NOT cartoon), the explosive 'YEW LEGEND "
        "ESHAYS' celebration moment told through the mid-air pose + dirt splash + dusk "
        "skyline — NO speech bubble, NO lightning-bolt overlays, NO cartoon prop icons, just "
        "the action moment"
    ),
    ...
    "westfield_boys": (
        ...
        "or dirty stubble, pale washed-out skin and sunburnt red necks, faded TN polos in red + navy "
        "+ black, navy tracksuit pants tucked into long white tube socks, white rubber slides on the "
        "feet, neck and finger tattoos, thin gold chains at the collar, slouched derro posture — "
        "GRITTY adult-faced lads matching Moey/Khoa NPC realism tier, NOT chibi NOT anime), "
        "some holding cans of Bundaberg rum, one passing around a glass bong, some laughing with "
        "mouths wide open, "
        "Brenno is HOLDING UP the brown paper bag from Khoa in one hand and a tall GLASS DOUBLE PERC "
        "BONG in the other, both raised over his head like trophies victorious, "
        "closed Westfield Mt Druitt mall facade silhouetted in the background — only the iconic "
        "WESTFIELD sign silhouette visible (NO readable text), "
        "low fluorescent ceiling lights spilling harsh white pools onto the asphalt, "
        ...
        "16-bit pixel art GRITTY realistic Western Sydney late-night eshay carpark sesh aesthetic, "
        "no close-up on individual faces, the rough crew + carpark vibe is the focus"
    ),
}
